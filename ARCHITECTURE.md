...
- `commonTest`: protocol, persistence, security, parser, HTTP, and operation-coordination tests
...
Android process-death restoration of unfinished form fields is not implemented in 3.4.0. Durable application state is restored from `Multiplatform Settings`; in-flight operations are intentionally not resumed.
...
- MQTT 3.1.1 is encoded/decoded in shared code and transported over WebSocket; PUBLISH payloads use strict UTF-8 detection and bounded binary HEX evidence.
- TCP/UDP adapters preserve exact bounded bytes for HEX evidence. Android/Desktop wrap blocking sockets in a cancellable bridge whose cancellation handler closes the active resource; iOS uses cancellable Ktor Network suspending sockets.
- CoAP maps the response Content-Format before selecting text or binary evidence.
- iOS bounded TCP capture uses a fixed byte array sized to the configured limit plus one truncation sentinel byte; no per-byte boxed collection is used.
...
Expected validation/capability failures are returned as user-readable results. `OperationCoordinator` serializes operations, joins the previous operation before publishing new state, and invalidates callbacks from obsolete operation IDs. Cancellation is not logged as an operation failure. Unexpected errors are redacted before durable logging. The application does not expose raw credentials in exception/history paths.
