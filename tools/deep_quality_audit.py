...
model_files=[str(path.relative_to(ROOT)) for path in sorted((ROOT/'composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/model').glob('*.kt'))]
model_text='\n'.join(read(path) for path in model_files)
for f in ('defaultHttpConnectTimeoutMillis','defaultHttpSocketTimeoutMillis','connectTimeoutMillis','socketTimeoutMillis','manualCookieHeader','customWebSocketHeaders','browserManagedRequestHeaders','manualHttpRedirects','publicCleartextOverride','httpConnectTimeout','httpSocketTimeout'):
    if f not in model_text: errors.append(f'core/model: missing {f}')
if 'redactSecretsInLogs' in model_text: errors.append('core/model: redaction must not be configurable')
...
validation='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/policy/ProtocolRuntimePolicy.kt'
for t in ('calculateHttpTimeoutBudget','HTTP total request timeout exceeded','remainingReceiveWindowMillis','validateWebSocketHeaders'): req(validation,t,'protocol runtime policy boundary missing')
preview='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/policy/HttpRequestTools.kt'
...
request_validation='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/policy/HttpRequestValidation.kt'
...
response_tools='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/policy/HttpResponseBodyTools.kt'
...
red='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/security/SecretRedactor.kt'
...
browser_probe='composeApp/src/webMain/kotlin/com/msa/iotofflinetoolbox/core/network/BrowserNetworkProbe.kt'
for t in ('manualCookieHeader = false','customWebSocketHeaders = false','browserManagedRequestHeaders = true','manualHttpRedirects = false','publicCleartextOverride = false','httpConnectTimeout = false','httpSocketTimeout = false'):
    req(browser_probe,t,'browser capability mismatch')
for source,suffix,platform in (('jsMain','js','Browser JS'),('wasmJsMain','wasmJs','Browser Wasm')):
    p=f'composeApp/src/{source}/kotlin/com/msa/iotofflinetoolbox/core/network/PlatformNetwork.{suffix}.kt'
    for t in ('BrowserNetworkProbe(', f'platform = "{platform}"'):
        req(p,t,'browser source-set composition mismatch')
...
protocol_operations='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/store/ToolboxProtocolOperations.kt'
operation_runner='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/store/ToolboxOperationRunner.kt'
for t in ('HttpRequestValidator.firstIssue','HttpRequestValidationPolicy(','capabilities.customWebSocketHeaders','capabilities.browserManagedRequestHeaders','capabilities.publicCleartextOverride'):
    req(protocol_operations,t,'Protocol application security enforcement missing')
for t in ('TransportSecurity.isPublicCleartext','requireCapability(','validationFailure('):
    req(operation_runner,t,'Shared operation policy missing')
settings_manager='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/store/ToolboxSettingsManager.kt'
req(settings_manager,'messages.localizeBackupInspection','Backup localization policy missing')
messages='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/store/ToolboxMessages.kt'
for t in ('class ToolboxMessages','fun text(','fun source(','fun httpValidationIssue(','fun localizeBackupInspection('):
    req(messages,t,'Application localization policy missing')
req(store, 'private val operationCoordinator = OperationCoordinator(operationScope)', 'Store operation coordinator ownership missing')
for t in ('coordinator.cancelCurrent()', 'coordinator.launch('):
    req(operation_runner, t, 'Operation coordinator integration missing')
...
template_inventory='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/store/ToolboxTemplateInventory.kt'
for t in ('val redactedContent = SecretRedactor.redact(template.content)', 'content = redactedContent'):
    req(template_inventory, t, 'payload template persistence must redact detected secrets')
mqtt_model='composeApp/src/commonMain/kotlin/com/msa/iotofflinetoolbox/core/model/ProtocolModels.kt'
